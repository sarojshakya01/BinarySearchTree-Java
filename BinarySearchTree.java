...
public class BinarySearchTree {
	...
	public void callInOrderSuccessor(Node treePoint) {
		...
		for (int i = 0; i < inorderList.size(); i++) {
			Node last = inOrderSuccesor(inorderList.get(i));
		}
	}
	...
	private void inOrderCyclic(Node treePoint) {
		...
		for (int i = 0; i < inorderList.size(); i++) {
			...
			if (!greater && treePoint == inorderList.get(i)){
				greater = true;
			}

			if (!greater) {
				preList.add(customerName(inorderList.get(i)));
			} else {
				postList.add(customerName(inorderList.get(i)));
			}
		}
		postList.addAll(preList);
		...
		while (!myStack.isEmpty()) { 
			...
			if (prev == null || prev.lLink == current || prev.rLink == current) { 
				if (current.lLink != null) 
					...
					myStack.pop(); 
					myList.add(customerName(current)); 
				}
			}  
